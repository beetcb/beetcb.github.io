...
nailimg: https://m.beetcb.com/nailimg/v8.png
tags: JS
---
### 首先从加载 js 说起
**浏览器如何加载 js 的**
...
现在许多流行的编译器都实现了JIT(just-in-time compiler)编译器，v8的JIT叫做 `TurboFan JIT`，他可以在 `RUNTIME` 实时编译出优化的机器代码，JIT分以下三个操作 => **三个操作各自占有自己的线程，可异步执行**
...
![](https://m.beetcb.com/postimg/15/1.png)
...
> 概括：
> ![](https://m.beetcb.com/postimg/15/2.png)
>    - Memory Heap
>    - Call Stack
>    - Web APIs
>    - Callback Queue
...
web apis不断验证等待执行的函数，当等待条件不在满足时，立刻抽出并加入到`callback queue`
